...
public class RecipeJpaRepositoryImpl implements RecipeJpaRepository {
	...
	public static final String bestMatchesQueryJPQL = "SELECT new kuehlfrank.backend.dto.RecipeSuggestion("
			...
			+ "(count(r.recipeId) - sum(case when i.common = true OR ie.ingredient.ingredientId <> NULL then 1 else 0 end)) AS missingIngredientCount, count(distinct ri.recipeIngredientId) AS totalIngredientCount"
			+ ") FROM Recipe r " + "left join RecipeIngredient ri on ri.recipeIngredientId.recipeId = r.recipeId "
			...
			+ "order by missingIngredientCount asc, r.recipeId";

	@Override
	public Collection<RecipeSuggestion> findBestMatchingRecipes(String userId, Pageable pageable) {
		TypedQuery<RecipeSuggestion> query = entityManager.createQuery(bestMatchesQueryJPQL, RecipeSuggestion.class)
				.setParameter("userId", userId).setFirstResult((int) pageable.getOffset())
	...
	private static final String detailedRecipeQueryJPQL = "SELECT new kuehlfrank.backend.dto.RecipeIngredientStatus("
			...
			+ "left join InventoryEntry ie on ie.inventory.inventoryId = u.inventory.inventoryId and ri.ingredient.ingredientId = ie.ingredient.ingredientId "
			+ "inner join Ingredient i on i.ingredientId = ri.ingredient.ingredientId "
			+ "where r.recipeId = :recipeId " + "order by ri.ingredient.ingredientId";

	@Override
	public DetailedRecipeSuggestion getDetailedRecipeSuggestion(UUID recipeId, String userId) {
		TypedQuery<RecipeIngredientStatus> ingredientStatusQuery = entityManager
				.createQuery(detailedRecipeQueryJPQL, RecipeIngredientStatus.class).setParameter("recipeId", recipeId)
				.setParameter("userId", userId);
		TypedQuery<Recipe> recipeQuery = entityManager
				.createQuery("SELECT r FROM Recipe r WHERE r.recipeId = :recipeId", Recipe.class)
		...
		for (RecipeIngredient ri : recipe.getRecipeIngredients()) {
			RecipeIngredientStatus recipeIngredientStatus = ingredientStatuses.stream()
					...
					.orElseThrow(() -> new RuntimeException("Could not find ingredient status"));

			if (recipeIngredientStatus.isMissing()) {
				missingIngredientsCount++;
			}

			recipeIngredients.add(new SuggestionRecipeIngredient(recipeIngredientStatus.isMissing(), ri.getIngredient(),
